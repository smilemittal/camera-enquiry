    ...
    <span aria-labelledby="btnSearchDrop12" class="dropdown-menu mt-1 dropdown-menu-right"
        ...
        style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(55px, 27px, 0px);">
            <a class="dropdown-item" href="{{ route('products.edit', $product ?? ''->id)}}"><i
              class="ft-edit-2"></i>{{translate('Edit')}}</a>
              <a href="#" data-form_id="{{ 'delete_form_'.$product->id }}" class="delete dropdown-item" ><i class="ft-trash-2"></i>{{translate('Delete')}}</a>
                        <form action="{{ $destroy }}" method="post" id="{{ 'delete_form_'.$product->id}}">  
                            @csrf  
                            @method('DELETE')  
                        </form>   
    </span>
