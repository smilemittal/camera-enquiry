<span class="dropdown">
    ...
    <span aria-labelledby="btnSearchDrop12" class="dropdown-menu mt-1 dropdown-menu-right" x-placement="bottom-end"
        ...
        <a href="#" data-form_id="{{ 'delete_form_' . $attribute_value->id }}" class="delete dropdown-item"><i
                class="ft-trash-2"></i>{{ translate('Delete') }}</a>
        <form action="{{ route('attribute-values.destroy', [$attribute_value->id]) }}" method="post"
            id="{{ 'delete_form_' . $attribute_value->id }}">
            @csrf
            @method('DELETE')
        </form>
    </span>
