 <span class="dropdown">
        ...
        <span aria-labelledby="btnSearchDrop12" class="dropdown-menu mt-1 dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(55px, 27px, 0px);"> 
            <a class="dropdown-item" href="{{ route('product-attributes.edit', [$productatrribute?? ''->id]) }}" >
                <i class="fa fa-edit"></i>Edit</a>
            <form action="{{ route('product-attributes.destroy', [$productatrribute->id]) }}" method="post" id="{{ 'delete_form_'.$productatrribute->id }}">
        @csrf 
        @method('DELETE')
        </form>
        <a class="dropdown-item" href="#" onclick="document.getElementById('{{ 'delete_form_'.$productatrribute->id }}').submit()">
