    ...
    $import_errors = [];
        foreach($rows as $row){
            ...
            if(((!empty($row['Model'])&&$row['Model'] != null) || (!empty($row['Product name'])&& $row['Product name'] != null))){
               ...
               if($system_type_id != '' && $type_id != '' && $standard_id != ''){
                ...
                foreach($row as $key => $value){
                    ...
                    if(($key != 'Model' && $key != 'Product name') && $key != 'Display order' && $key != 'Price' && $key != 'Priority' && $key != 'System type' && $key != 'Type' && ($key != 'Standards' && $key != 'Standard') && $value != null && $value != '' && $system_type_id != '' && $type_id != '' && $standard_id != ''){
          // dd($row['System type'], $standard_id, $type_id, $system_type_id, $this->products_imported);
                    ...
                    $latest_attribute_value = AttributeValue::where('attribute_id', $attribute_id)->where('system_type_id' , '=', $system_type_id)->where('type_id', $type_id)->where('standard_id', $standard_id)->orderBy('id', 'DESC')->first();
                    ...
                    $attribute_values= AttributeValue::where('attribute_id', $attribute_id)->where('value', 'LIKE', $value)->where('type_id', $type_id)->where('standard_id', $standard_id)->first();

                    if(!$attribute_values){
                        $attribute_value = AttributeValue::create(['attribute_id' => $attribute_id, 'value' => $value ,'display_order'=>$attribute_value_display_order , 'system_type_id' => $system_type_id ,'type_id'=> $type_id, 'standard_id' => $standard_id]);
